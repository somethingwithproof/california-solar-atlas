...
function capacityBasisNote(city) {
  if (!city.pouCapacity) return 'Positive System Size DC values only; PTC and CEC-AC values are not substituted or mixed into totals.';
  return `IOU project records are DC; ${escapeHtml(city.pouCapacity.utility)} reports ${escapeHtml(city.pouCapacity.basis)} and is converted across an inverter-loading-ratio band, so the total is a range.`;
}
