...
import { capacityFloor, format, generationMid, generationRange, integer, metricConfig, metricDisplay } from './metrics.js';
...
async function start() {
  try {
    ...
    if (state.data.meta?.schemaVersion !== 8) throw new Error(`This build expects data schema 8 but received ${state.data.meta?.schemaVersion ?? 'none'}. The page and the data file are out of step.`);
