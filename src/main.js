  ...
  app.innerHTML = `
    ...
    <main id="main">
      ...
      <section id="map" class="map-section section-pad">
        <div class="section-heading"><div><div class="eyebrow"><span></span> Statewide view</div><h2>See the pattern.</h2></div><p>Official incorporated-city polygons are shaded by the selected metric, including attributed municipal capacity. Values still use utility service-city strings, so the map visualizes—but does not repair—the mailing-boundary mismatch.</p></div>
      ...
      <section id="compare" class="compare section-pad">
        <div class="section-heading"><div><div class="eyebrow"><span></span> Side by side</div><h2>Compare cities.</h2></div><p>Add up to four cities. Every comparison keeps the same capacity basis, degradation assumption, and location-specific yield method, and includes attributed municipal capacity where it exists.</p></div>
      ...
      <section class="method section-pad" id="methodology">
        ...
        <div class="method-grid">
          <article><span>01</span><h3>Reported capacity</h3><p>Positive System Size DC values from PG&amp;E, SCE, and SDG&amp;E project-site files; PTC and CEC-AC values are never substituted for them. Attributed municipal capacity is added on top from EIA-861 and converted to DC across a disclosed ratio band, which is why those cities show a range.</p></article>
...
function limitsDialog(meta) {
  ...
  return `<dialog class="limits-dialog" aria-labelledby="limits-title"><form method="dialog"><button class="dialog-close" aria-label="Close data limitations">×</button></form><div class="eyebrow"><span></span> Data limitations</div><h2 id="limits-title">What this data can—and cannot—tell you.</h2><div class="limits-list">
    ...
    <article><b>Utility coverage varies.</b><p>Project records cover PG&amp;E, SCE, and SDG&amp;E only. Municipal capacity is added for ${integer.format(pou.mergedCities)} cities from ${escapeHtml(pou.year)} Form EIA-861 (${format.format(pou.mergedRangeMwDc.low)}–${format.format(pou.mergedRangeMwDc.high)} MW-DC) where the utility's service territory lies inside the city. A further ${format.format(pou.unattributedReportedMw)} MW across ${integer.format(pou.unattributedUtilities)} utilities, including SMUD and Imperial Irrigation District, serves several cities at once and is not attributed to any of them. Project counts, sector splits, and growth curves remain PG&amp;E, SCE, and SDG&amp;E only for every city.</p></article>
...
function selectCity(city, { scroll = true, historyMode = 'push' } = {}) {
  ...
  view.innerHTML = `<div class="city-heading"><div><div class="eyebrow"><span></span>${escapeHtml(city.county)} County · ${term('CEC climate zone', 'climateZone')} ${zoneLabel(city)}</div><h2>${escapeHtml(city.name)}</h2></div><div class="city-actions">${qualityBadge(city)}${geographyBadge(city)}<button data-action="share">Share</button><button data-action="csv">Download CSV</button></div></div>
    ...
    <div class="provenance"><span>Geography</span><p>Utility service-city string (mailing geography), not a municipal polygon join.</p><span>Capacity basis</span><p>${city.pouCapacity ? `IOU project records are DC; ${escapeHtml(city.pouCapacity.utility)} reports ${escapeHtml(city.pouCapacity.basis)} and is converted across an inverter-loading-ratio band, so the total is a range.` : 'Positive System Size DC values only; PTC and CEC-AC values are not substituted or mixed into totals.'}</p><span>History quality</span><p>Application-approval-date proxy; PTO generally occurs later. Missing dates are kept out of the timeline and widen the generation range.</p>${loadSource(city)}</div>`;
